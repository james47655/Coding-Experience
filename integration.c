// James Whitescarver 361 lab 7 intergration code
#include <math.h>
#include <stdlib.h>
#include <stdio.h>
// Declare functions
...
double midpoint(double (*integrand)(double), double a, double b){
  return (b-a)*integrand((double) (a+b)/2);
}
double simpson_13(double (*integrand)(double), double a, double b){
  return ((b-a)/6)*(integrand((double) a)+4*(integrand((double) (a+b)/2))+integrand((double) b));
}
double simpson_38(double (*integrand)(double), double a, double b){
  return ((b-a)/8)*(integrand((double) a)+3*(integrand((double) (2*a+b)/3))+3*(integrand((double) (a+2*b)/3))+integrand((double) b));
}
double gauss_quad(double (*integrand)(double), double a, double b, int n){
  ...
  for(int i = 1; i<=n; i++)
    {
      sum += w[n][i] * integrand((double) c + (m*t[n][i]));
    }
  return sum*m;
}
